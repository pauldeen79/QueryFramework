...
namespace QueryFramework.InMemory
{
    internal static class Functions
    {
        internal static readonly Dictionary<string, Func<object?, IEnumerable<string>, object>> Items = new Dictionary<string, Func<object?, IEnumerable<string>, object>>(StringComparer.CurrentCultureIgnoreCase)
        {
            ...
            { "LEFT", (value, arguments) => GetStringValue(value).Substring(0, int.TryParse(arguments.FirstOrDefault() ?? string.Empty, NumberStyles.Any, CultureInfo.InvariantCulture, out int length) ? length : 0) },
            { "RIGHT", (value, arguments) =>
                {
                    var s = GetStringValue(value);
                    return s.Substring(s.Length - (int.TryParse(arguments.FirstOrDefault() ?? string.Empty, NumberStyles.Any, CultureInfo.InvariantCulture, out int length) ? length : 0));
                } },
