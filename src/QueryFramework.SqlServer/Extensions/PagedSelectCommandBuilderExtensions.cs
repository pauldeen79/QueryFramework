...
internal static class PagedSelectCommandBuilderExtensions
{
                                                           ...
                                                           object? context)
    {
        foreach (var querySortOrder in orderByFields.Select((x, index) => new { Item = x, Index = index }))
        {
            ...
            instance.OrderBy($"{evaluator.GetSqlExpression(querySortOrder.Item.FieldNameExpression.ToUntyped(), fieldInfo, parameterBag, context)} {querySortOrder.Item.ToSql()}");
        }
