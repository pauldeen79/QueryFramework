...
public static class QueryExpressionBuilderExtensions
{
    ...
    public static IQueryExpressionBuilder Coalesce(this IQueryExpressionBuilder instance, params IQueryExpressionBuilder[] innerExpressions)
        => instance.Chain(x => x.Function = new CoalesceFunctionBuilder().WithFunction(instance.Function)
                                                                         .AddInnerExpressions(innerExpressions)
                                                                         .WithFieldName(instance.FieldName));

    /// <summary>Gets the first non-null value of the specified fields.</summary>
    public static IQueryExpressionBuilder Coalesce(this IQueryExpressionBuilder instance, IEnumerable<IQueryExpressionBuilder> innerExpressions)
    ...
    public static IQueryExpressionBuilder Coalesce(this IQueryExpressionBuilder instance, params string[] innerFieldNames)
        => Coalesce(instance, innerFieldNames.Select(innerFieldName => new QueryExpressionBuilder().WithFieldName(innerFieldName)));
